...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
            <div>
                <div style="width:100%;" >
                    <table class="w-100 d-block d-md-table table-partitur">
                        <theader>
                            <tr>
    ...
    <xsl:template match="event">
        ...
        <!-- check if "empty" must be inserted BEFORE the event -->
        <xsl:choose>
            ...
            <xsl:when test="preceding-sibling::event[1]">
                ...
                <!-- ... and the preceding event does not end where the current one starts -->
        ...
        <td>
            ...
            <xsl:apply-templates>
                <xsl:with-param name="START" select="$TIMELINE_COPY/descendant::tli[@id=$START]/@time"/>
                <xsl:with-param name="END" select="$TIMELINE_COPY/descendant::tli[@id=$END]/@time"/>
            </xsl:apply-templates>
    ...
    <xsl:function name="exmaralda:tierSorter" as="xs:integer">
        ...
        <xsl:variable name="TYPE_WEIGHT" as="xs:integer">
            <xsl:choose>
                ...
                <xsl:when test="$TIER/@type='a' and $TIER/@category='comment'">4</xsl:when>
                <xsl:when test="$TIER/@type='d'">6</xsl:when>
                <xsl:when test="$TIER/@type='a'">8</xsl:when>
